...
import { MAX_TRACKS_FOR_CURATION, type Curation, type LibraryTrack } from "@/lib/types";
...
Listening tags: recent = played lately, top_short ≈ last month, top_medium ≈ 6 months, top_long ≈ years, saved = liked songs. plays=N is a lifetime play count, when known.`;
...
export function formatTrackLine(t: LibraryTrack, index: number): string {
  ...
  if (t.playCount) line += ` plays=${t.playCount}`;
  if (t.sources.length) line += ` {${t.sources.join(",")}}`;
  return line;
}

// Tracks that show up in more places, get played more, or persist long-term matter more to the user.
export function prioritizeTracks(tracks: LibraryTrack[], cap = MAX_TRACKS_FOR_CURATION): LibraryTrack[] {
  ...
  const score = (t: LibraryTrack) =>
    t.sources.length * 2 + (t.sources.includes("top_long") ? 1 : 0) + Math.log2(1 + (t.playCount ?? 0));
...
/** Fails fast, before any rate-limit budget is spent, when no Anthropic credentials exist. */
export function assertCuratorConfigured() {
  if (!process.env.ANTHROPIC_API_KEY && !process.env.ANTHROPIC_AUTH_TOKEN) {
    throw new AppError("CURATOR_MISCONFIGURED", MISCONFIGURED_MESSAGE, 500);
  }
}
